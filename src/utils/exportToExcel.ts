...
export async function exportAbsensiToExcel(data: any, filename: string = "Laporan_Absensi") {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("Laporan Absensi");
  ...
  let normalizedData: AbsensiByDate = {};

  // Check if data is array (flat structure) or object (grouped by date)
  if (Array.isArray(data)) {
    // If data is array, group by date
    data.forEach((record: AbsensiData) => {
      const date = new Date(record.absenMasukAt).toISOString().split("T")[0];
      if (!normalizedData[date]) {
        normalizedData[date] = [];
      }
      normalizedData[date].push(record);
    });
  } else if (data && typeof data === "object") {
    // If data is already grouped by date, use it directly
    normalizedData = data;
  } else {
  ...
  // Add data
  let rowNumber = 1;
  const sortedDates = Object.keys(normalizedData).sort((a, b) => b.localeCompare(a));

  for (const date of sortedDates) {
    const records = normalizedData[date];

    // Skip if not array or empty
    if (!Array.isArray(records) || records.length === 0) {
      console.warn(`Skipping invalid data for date ${date}:`, records);
      continue;
    }
    ...
    for (const record of records) {
      try {
        const row = worksheet.addRow({
          ...
          userId: record.userId?.toString() || "-",
          statusMasuk: record.statusMasuk === "ON_TIME" ? "Tepat Waktu" : "Terlambat",
          absenMasukAt: record.absenMasukAt
            ? new Date(record.absenMasukAt).toLocaleString("id-ID", {
                hour: "2-digit",
                minute: "2-digit",
                second: "2-digit",
                timeZone: "Asia/Makassar",
            ...
            : "-",
          lokasiMasuk:
            record.latitudeMasuk && record.longitudeMasuk ? `${record.latitudeMasuk}, ${record.longitudeMasuk}` : "-",
          absenPulangAt: record.absenPulangAt
            ? new Date(record.absenPulangAt).toLocaleString("id-ID", {
                hour: "2-digit",
                minute: "2-digit",
            ...
            : "-",
          lokasiPulang:
            record.latitudePulang && record.longitudePulang
              ? `${record.latitudePulang}, ${record.longitudePulang}`
              : "-",
        });
